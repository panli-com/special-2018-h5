...
function FloorAll(data, imgHead, id, mlc) {
  ...
  for (var i = 0; i < data.length; i++) {
    var name = data[i].店铺名称,
      ...
      shopImg = "./build/css/img/list/s" + (i+1) + '.png',
      // Url = data[i].商品导购,
      shopUrl = data[i].店铺链接;
    ...
    str += `
      <li>
          ...
          <div class="shop"><img src="${shopImg}" alt="" srcset="" /><span>${name}</span></div>
          <div class="more"><a href="${shopUrl}"  target="_blank"><span>进店抢购</span></a></div>
      </li>
      `;
  }
  PD("#ul" + id)
